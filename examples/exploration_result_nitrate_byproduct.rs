//! Nitrate byproduct support: re-runs the exact same Phase 28/29 recall
//! measurement as `exploration_recall_result_v0_7_0.rs`, against the
//! same frozen catalog, with `curated_byproducts()`'s new NO2 entry
//! (`src/balance.rs`) now in place. The only code difference from that
//! prior run is the byproduct-allow-list widening -- everything else
//! (search algorithm, catalog, budget) is unchanged, so a target-by-
//! target diff against the committed v0.7.0 result isolates exactly
//! what this one change recovers.
//!
//! Run: `cargo run --release --example exploration_result_nitrate_byproduct
...
//!
//! Writes `benchmarks/data/exploration_result_nitrate_byproduct.json` --
//! a **separate** file from both the committed, immutable
//! `exploration_baseline_v0_6_0.json` and `exploration_result_v0_7_0.json`,
//! never overwriting either. Chains back to the catalog and the v0.6.0
//! baseline via sha256 checksums, same convention as
//! `exploration_recall_result_v0_7_0.rs`.
...
const PRIOR_RESULT_PATH: &str = "benchmarks/data/exploration_result_v0_7_0.json";
const OUTPUT_PATH: &str = "benchmarks/data/exploration_result_nitrate_byproduct.json";
...
/// alone -- the owner-confirmed expectation for this fix is a
/// single-digit target recovery, not a further +20%.
...
fn main() {
    ...
    println!("gugen (nitrate byproduct fix, on top of Phase 29) exploration recall result");
    ...
    println!(
        "this fix is not gated on that threshold -- see the diff vs. v0.7.0 below for its \
        real, isolated effect"
    );
    ...
    // Direct row-by-row diff against the committed v0.7.0 result (same
    // catalog, same search algorithm, the only code difference is
    // curated_byproducts()'s new NO2 entry) -- this isolates exactly
    ...
    let prior_json: serde_json::Value =
        serde_json::from_str(&prior_raw).expect("committed v0.7.0 result must be valid JSON");
    let prior_rows: &Vec<serde_json::Value> = prior_json["rows"]
        .as_array()
        .expect("committed v0.7.0 result must have a rows array");
    assert_eq!(
        prior_rows.len(),
        results.len(),
    ...
    println!(
        "diff vs. committed v0.7.0 result (isolates this fix's effect): \
    ...
    out.push_str("{\n");
    out.push_str(
        "  \"description\": \"nitrate-byproduct-fix exploration recall result -- not a gated \
        change, this file reports the real measured effect of adding NO2 to \
        curated_byproducts() on top of Phase 29's frontier search\",\n",
    );
    ...
    out.push_str(&format!(
        "  \"newly_recovered_vs_v0_7_0_count\": {},\n",
        newly_recovered.len()
    ));
    out.push_str(&format!(
        "  \"newly_lost_vs_v0_7_0_count\": {},\n",
        newly_lost.len()
    ));
    out.push_str(&format!(
        "  \"newly_recovered_vs_v0_7_0_targets\": {},\n",
        serde_json::to_string(&newly_recovered).unwrap()
    ));
    out.push_str(&format!(
        "  \"newly_lost_vs_v0_7_0_targets\": {},\n",
