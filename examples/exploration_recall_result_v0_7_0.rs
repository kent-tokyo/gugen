//! Phase 29 (Guided Precursor Frontier): computes precursor-set recall
//! against the same frozen catalog `examples/exploration_recall_baseline.rs`
//! measured, with the new best-first frontier search now in place, and
//! checks the result against the gate the owner confirmed from Phase
//! 28's real numbers (relative +20% on overall recall: 0.4257 -> ≥0.5112).
//!
//! Run: `cargo run --release --example exploration_recall_result_v0_7_0
...
//! the exact commands) -- release mode matters here, not just for
//! comfort: this ran for minutes unoptimized on the real 2879-row
//! catalog during Phase 28.
//!
//! Writes `benchmarks/data/exploration_result_v0_7_0.json` -- a
//! **separate** file from the committed, immutable
//! `exploration_baseline_v0_6_0.json`, never overwriting it (per that
//! file's own doc comment). Chains back to both the catalog and the
//! v0.6.0 baseline via sha256 checksums, mirroring
//! `analyze_oqmd_coverage_gate.py`'s own `source_manifest_checksum`
//! convention for verifiable-against-a-frozen-artifact results.
...
const OUTPUT_PATH: &str = "benchmarks/data/exploration_result_v0_7_0.json";
...
/// record for the full confirmation trail.
...
fn main() {
    ...
    println!("gugen (post-Phase-29) exploration recall result");
    ...
    println!(
        "baseline (R0, v0.6.0): {BASELINE_RECALL:.4} -- required (relative +20%): \
        {required_recall:.4}"
    );
    ...
    out.push_str("{\n");
    out.push_str("  \"description\": \"post-Phase-29 guided frontier search result\",\n");
    ...
    out.push_str(&format!("  \"gate_multiplier\": {GATE_MULTIPLIER:.2},\n"));
    out.push_str(&format!("  \"required_recall\": {required_recall:.6},\n"));
