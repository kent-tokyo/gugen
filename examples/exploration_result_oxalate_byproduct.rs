//! Oxalate byproduct support: re-runs the exact same exploration recall
//! measurement as `exploration_result_nitrate_byproduct.rs`, against the
//! same frozen catalog, with `curated_byproducts()`'s new CO entry
//! (`src/balance.rs`) now also in place (on top of NO2, already merged).
//! The only code difference from the nitrate result is the CO addition
//! -- everything else (search algorithm, catalog, budget) is unchanged,
//! so a row-position diff against the committed **post-nitrate**
//! result isolates exactly what this one change recovers. Deliberately
//! diffs against `exploration_result_nitrate_byproduct.json`, not the
//! older `exploration_result_v0_7_0.json` -- that file predates the
//! nitrate fix and would conflate the two gains.
//!
//! Run: `cargo run --release --example exploration_result_oxalate_byproduct
...
//!
//! Writes `benchmarks/data/exploration_result_oxalate_byproduct.json` --
//! a **separate** file from the committed, immutable
//! `exploration_baseline_v0_6_0.json` and `exploration_result_nitrate_byproduct.json`,
//! never overwriting either. Chains back to the catalog and the v0.6.0
//! baseline via sha256 checksums, same convention as
//! `exploration_result_nitrate_byproduct.rs`.
...
const PRIOR_RESULT_PATH: &str = "benchmarks/data/exploration_result_nitrate_byproduct.json";
const OUTPUT_PATH: &str = "benchmarks/data/exploration_result_oxalate_byproduct.json";
...
/// alone -- no target expectation is pre-registered for this fix (the
/// exploration corpus's own oxalate-precursor count hasn't been
/// separately measured; report whatever the real number is).
...
fn main() {
    ...
    println!(
        "gugen (oxalate byproduct fix, on top of Phase 29 + nitrate fix) exploration recall result"
    );
    ...
    println!(
        "this fix is not gated on that threshold -- see the diff vs. the nitrate-fix result \
    ...
    // Direct row-by-row diff against the committed nitrate-fix result
    // (same catalog, same search algorithm, the only code difference is
    // curated_byproducts()'s new CO entry) -- this isolates exactly
    // what this fix recovers, more precisely than any estimate made
    // ahead of running it.
    //
    ...
    let prior_json: serde_json::Value =
        serde_json::from_str(&prior_raw).expect("committed nitrate-fix result must be valid JSON");
    let prior_rows: &Vec<serde_json::Value> = prior_json["rows"]
        .as_array()
        .expect("committed nitrate-fix result must have a rows array");
    assert_eq!(
        prior_rows.len(),
        results.len(),
    ...
    println!(
        "diff vs. committed nitrate-fix result (isolates this fix's effect): \
    ...
    out.push_str("{\n");
    out.push_str(
        "  \"description\": \"oxalate-byproduct-fix exploration recall result -- not a gated \
        change, this file reports the real measured effect of adding CO to \
        curated_byproducts() on top of Phase 29's frontier search and the merged nitrate fix\",\n",
    );
    ...
    out.push_str(&format!(
        "  \"newly_recovered_vs_nitrate_fix_count\": {},\n",
        newly_recovered.len()
    ));
    out.push_str(&format!(
        "  \"newly_lost_vs_nitrate_fix_count\": {},\n",
        newly_lost.len()
    ));
    out.push_str(&format!(
        "  \"newly_recovered_vs_nitrate_fix_targets\": {},\n",
        serde_json::to_string(&newly_recovered).unwrap()
    ));
    out.push_str(&format!(
        "  \"newly_lost_vs_nitrate_fix_targets\": {},\n",
